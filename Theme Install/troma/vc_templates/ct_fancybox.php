...
$bg_image_url = '';
if (!empty($bg_image)) {
    $bg_attachment_image = wp_get_attachment_image_src($bg_image, 'full');
    $bg_image_url = $bg_attachment_image[0];
}
...
?>
<div class="ct-fancybox-default bg-image <?php echo esc_attr($el_class.' '.$animation_classes); ?>" <?php if(!empty($bg_image_url)) : ?>style="background-image: url(<?php echo esc_url($bg_image_url); ?>);"<?php endif; ?>>
	<div class="ct-fancybox-inner">
        ...
        <?php } else { ?>
            <?php if($icon_class):?>
                <div class="ct-fancybox-icon">
                    <i class="<?php echo esc_attr($icon_class); ?> text-gradient2" style="<?php if(!empty($icon_color)) { echo 'color:'.esc_attr($icon_color).';'; } if(!empty($icon_font_size)) { echo 'font-size:'.esc_attr($icon_font_size).'px;'; } ?>"></i>
                </div>
            <?php endif;?>
        <?php } ?>
        ...
        <div class="ct-fancybox-content">
            ...
            <?php if(!empty($a_href)) : ?>
                <div class="ct-fancybox-more">
                    <a class="btn" href="<?php echo esc_url($a_href);?>" target="<?php  echo esc_attr($a_target); ?>"><i class="fa fa-angle-double-right"></i></a>
                </div>
            <?php endif; ?>
        </div>
	</div>
